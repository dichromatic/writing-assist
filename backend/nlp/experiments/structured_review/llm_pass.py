...
def run_dossier_llm_pass(
    ...
    *,
    ...
    """Compatibility wrapper for the earlier dossier-specific LLM function.
    ...
    Returns:
        Updated bundle with completed or failed LLM fields.
    """
    return run_structured_llm_pass(
        bundle,
        model=model,
        responder=responder,
        api_key=api_key,
...
def run_dossier_llm_passes(
    ...
    *,
    ...
    """Compatibility wrapper for the earlier dossier-specific batch function.

    Args:
        bundles: Deterministic structured review bundles.
        model: Model name to use.
        max_records: Optional cap on records sent to the model.
        responder: Optional injected responder for tests.
        api_key: Optional explicit API key.
        base_url: Provider endpoint.
        timeout_seconds: HTTP timeout for live calls.
    ...
    Returns:
        Updated bundles with completed or failed LLM fields.
    """
    return run_structured_llm_passes(
        bundles,
        model=model,
