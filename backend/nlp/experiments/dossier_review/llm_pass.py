"""
Dossier LLM pass - run the first constrained model extraction over one bundle.
...
def _response_schema() -> dict[str, Any]:
    """Return the structured output schema for the first dossier LLM pass.

    Returns:
        JSON Schema object for constrained dossier subject and fact extraction.
    """
    return {
        "name": "dossier_subject_and_explicit_facts",
        "strict": True,
        "schema": {
            "type": "object",
...
def _build_request_payload(bundle: RecordReviewBundle, model: str) -> dict[str, Any]:
    """Build the Responses API request body for one dossier bundle.
    ...
    system_text = "\n".join([
        "You extract dossier subject and explicit subject facts from one structured record.",
...
def _build_chat_completions_payload(bundle: RecordReviewBundle, model: str) -> dict[str, Any]:
    """Build the chat-completions request body for one dossier bundle.
...
def _call_openai_responses_api(
    ...
    *,
    ...
    """Call the OpenAI Responses API for one dossier bundle.
...
def run_dossier_llm_pass(
    ...
    *,
    ...
    """Run the first constrained dossier LLM pass for one review bundle.

    Args:
        bundle: Deterministic dossier review bundle.
...
def run_dossier_llm_passes(
    ...
    *,
    ...
    """Run the first constrained LLM pass over many dossier bundles.

    Args:
        bundles: Deterministic dossier review bundles.
        model: Model name to use.
    ...
    for index, bundle in enumerate(bundles):
        if max_records is not None and index >= max_records:
            ...
            continue
        updated.append(run_dossier_llm_pass(
            bundle,
            model=model,
            responder=responder,
            api_key=api_key,
