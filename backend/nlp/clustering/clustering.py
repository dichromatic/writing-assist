...
def cluster_mentions(candidates: list[MentionCandidate]) -> list[MentionCluster]:
    ...
    for normalized_key, group in groups.items():
        ...
        clusters.append(MentionCluster(
            ...
            title_support_count=sum(1 for c in group if c.has_title_prefix),
            possessive_support_count=sum(1 for c in group if c.has_possessive),
            location_support_count=sum(1 for c in group if c.has_location_context),
            linked_fields=[],
            linked_definitions=[],
            linked_seeds=[],
